...
class v7(pl.LightningModule):
    def __init__(self, channels=3, dims=(64, 256, 512, 1024), n_heads=(1, 2, 8, 8), expansion=(8, 8, 4, 4), reduction_ratio=(8, 4, 2, 1), n_layers=(2, 2, 2, 2), learning_rate = config.LEARNING_RATE):
        ...
        self.to_segment_conv = nn.Conv2d(4, 1, 1)

        # self.reduce_channels_1 = DoubleConv(dims[0], dims[0])
        # self.reduce_channels_2 = DoubleConv(dims[1], dims[1])
        # self.reduce_channels_3 = DoubleConv(dims[2], dims[2])
        # self.reduce_channels_4 = DoubleConv(dims[3], dims[3])
    ...
    def forward(self, x):
        ...
        up_side_1 = self.upsampling_1(mit_1)
        up_side_2 = self.upsampling_2(mit_2)
        up_side_3 = self.upsampling_3(mit_3)
        up_side_4 = self.upsampling_4(mit_4)

        to_fused = torch.concat((up_side_1, up_side_2, up_side_3, up_side_4),dim=1)
        to_segment = self.to_segment_conv(to_fused)

        return to_segment, up_side_1, up_side_2, up_side_3, up_side_4
